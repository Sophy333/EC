...
// -------------------- Greedy LS (randomized neighborhood scanning) --------------------
static py::tuple greedy_full_cpp(
    ...
    std::vector<int> pos_order(m);
    std::vector<int> i_order(m);
    std::vector<int> unselected;
    unselected.reserve(n);

    for (int i = 0; i < m; i++) { pos_order[i] = i; i_order[i] = i; }
    ...
    while (improved) {
        ...
        std::shuffle(i_order.begin(), i_order.end(), rng);
        std::shuffle(unselected.begin(), unselected.end(), rng);

        bool intra_first = std::uniform_int_distribution<int>(0, 1)(rng) == 0;

        auto try_intra = [&]() -> bool {
            for (int ii : i_order) {
                // build j candidates and shuffle
                std::vector<int> j_cand;
                j_cand.reserve(m);
                for (int j = ii + 2; j < m; j++) {
                    if (ii == 0 && j == m - 1) continue;
                    j_cand.push_back(j);
                }
                std::shuffle(j_cand.begin(), j_cand.end(), rng);

                for (int j : j_cand) {
                    evaluated++;
                    long long d = delta_two_opt(dist, n, tour, ii, j);
                    if (d < 0) {
                        apply_two_opt(tour, ii, j);
                        return true;
                    }
                }
            }
...
PYBIND11_MODULE(ec_ls, m) {
    m.doc() = "EC: steepest LS + greedy LS (randomized order)";
    ...
    m.def("greedy_full", &greedy_full_cpp,
          py::arg("dist"), py::arg("costs"), py::arg("seed") = 123);
}
